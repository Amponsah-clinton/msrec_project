document.addEventListener("DOMContentLoaded", () => {
  ...
  const REMOVE_ICON = '<svg viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12"/></svg>';
  ...
  addTeamRowBtn.addEventListener("click", () => {
    ...
    row.innerHTML = `
      ...
      <button type="button" class="apply-team-remove" aria-label="Remove row">${REMOVE_ICON}</button>
    `;
  ...
  // The dashboard topbar sits in normal flow (not fixed over the content
  // the way the public site's navbar is), so .apply-section only needs a
  // small scroll-margin-top here — keep this in step with that value.
  const SCROLLSPY_TRIGGER_OFFSET = 40;
