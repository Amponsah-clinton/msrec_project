"""Question battery for the MSREC Assistant. Each case: (role, question,
[any-of groups that must each be matched (case-insensitive)], [forbidden]).
Run inside `manage.py shell`. Uses the live model providers."""
import json
import re
import sys
import time
...
from assistant import knowledge, llm
...
def ask(role, question):
    ...
    system = knowledge.build_system_prompt(req, {"title": "Dashboard", "path": "/"}, [], msgs)
    reply, provider = llm.complete(system, msgs)
    for _ in range(3):
        if reply:
            break
        time.sleep(25)  # every provider was rate-limited; wait it out
        reply, provider = llm.complete(system, msgs)
    return reply, provider, len(system)


def run(only=None):
    passed, failed = 0, []
    for i, (role, q, groups, forbidden) in enumerate(CASES):
        if only and i not in only:
            continue
        reply, provider, size = ask(role, q)
        text = (reply or "").lower()
        for a, b in (("’", "'"), ("‘", "'"), ("‑", "-"), ("‐", "-"), ("–", "-"), (" ", " ")):
            text = text.replace(a, b)
        missing = [g for g in groups if not any(k.lower() in text for k in g)]
        bad = [f for f in forbidden if f.lower() in text]
        ok = reply and not missing and not bad
        if ok:
            passed += 1
        else:
            failed.append((i, role, q, missing, bad, reply))
        print(f"{'OK ' if ok else 'BAD'} [{role or 'public'}] {q}  ({provider}, ~{size//4} tok)", flush=True)
        time.sleep(4.5)
    print(f"\nPASSED {passed}/{passed + len(failed)}")
    for i, role, q, missing, bad, reply in failed:
        print(f"\n--- #{i} [{role or 'public'}] {q}\n   missing: {missing} forbidden-hit: {bad}\n   reply: {(reply or '')[:600]}")
    return failed
