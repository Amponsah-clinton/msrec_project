...
Letterhead: upload a header image (top of page 1) and a footer image (bottom of every page) for the approval letter; they're placed edge to edge on A4 (about 2480 px wide; header max height 55 mm, footer 38 mm, larger images are scaled down; blank margins trimmed). Letter signatory: name, title and signature image for the letter, or leave blank to use the Chair. Uploaded images are saved to Supabase Storage and the old file is deleted on replace/remove.""", frozenset({"admin"})),
    C("admin-maintenance", "Admin: putting the site under maintenance",
      "maintenance,maintenance mode,lock the site,take the site offline,site offline,downtime,under maintenance,schedule maintenance,close the site,shut down site,maintenance page,reopen site,end maintenance",
      """Site Settings > Maintenance locks the whole site for everyone except administrators. "Start maintenance now" locks it immediately; "End maintenance now" reopens it. Or use the Maintenance mode switch with a Starts and Ends time (GMT) and an optional message, then Save — with an end time the site reopens by itself, and before the start time it stays open. "Preview page" shows exactly what visitors will see. While it's on, every page and dashboard — for everyone, administrators included — shows the maintenance page (with the under-maintenance illustration, the schedule and a countdown). Only the /admins/ area and the login pages stay open, so an administrator signs in and goes to Site Settings > Maintenance to end it; inside the admin area a small "Maintenance mode is on" bar reminds them. A payment already in progress can still be confirmed. The settings are stored in the database (the site_settings row).""", frozenset({"admin"})),
